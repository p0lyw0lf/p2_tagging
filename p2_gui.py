...
from p2_scrape import ScrapeHandler
...
class MultiStringVars:
    ...
    def delete_specific_entry(self, var, entry):
        ...
        """
        def _delete_helper():
            # FIXME: comment the below out once it's no longer needed
            print(entry)
...
def main():
    ...
    filemenu.add_command(label="Load Metadata", command=lambda: print("Not implemented!"))
    filemenu.add_separator()
    ...
    USAGE_TEXT = """Yet to be written"""
    HELP_TEXT = """Also yet to be written"""
    ...
    def _focusNext(widget):
        ...
        widget = root.call('tk_focusNext', widget._w)
        if not widget: return None
        return root.nametowidget(widget.string)

    def OnTextTab(event):
        '''Move focus to next widget'''
        widget = event.widget
        next_widget = _focusNext(widget)
        next_widget.focus()
        return "break"
