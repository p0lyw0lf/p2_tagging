import io
import os
from tkinter import Tk, Menu, StringVar, N, S, E, W
from tkinter import ttk, filedialog, simpledialog

from mutagen.mp4 import AtomDataType
from PIL import Image, ImageTk

from tagging import Song
from scrape import (ScrapeHandler, load_image_from_url, SpotifyScraper,
                    SoundcloudScraper)
...
class MultiStringVars:
    ...
    def add_button(self):
        self.new_entry_button = ttk.Button(self.container,
                                           text="+",
    ...
    def delete_specific_entry(self, var, entry):
        ...
        def _delete_helper():
            ...
            self.var_entries = [(frame, t, d)
                                for frame, t, d in self.var_entries
                                if frame != entry]
    ...
    def delete_last_entry(self):
        ...
        if i >= 0:
            self.delete_specific_entry(self.vars[i], self.var_entries[i])()
...
class SongInput:
    ...
    def __init__(self, container, r=0):
        ...
        self.image_button.bind("<Button-2>",
                               lambda _: self.set_image_rightclick())
        self.image_button.bind("<Button-3>",
                               lambda _: self.set_image_rightclick())
    ...
    def set_image_rightclick(self):
        url = simpledialog.askstring(
            "Load Image URL", "Enter a URL to load album artwork from:")
...
class SongLoader:
    ...
    def set_directory(self):
        directory = filedialog.askdirectory(title="Choose Music Directory",
    ...
    def refresh_filelist(self):
        ...
        self.file_list = [
            filename for filename in unfiltered_list
    ...
    def copy_song_info_to_album_info(self):
        if self.current_song is not None:
            self.current_song.album = self.current_song.title
            self.current_song.album_artists = self.current_song.artists
            self.current_song.save_tags()
            self.update_current_song(self.current_song)
    ...
    def set_index(self):
        ...
        if new_index and 0 <= new_index < len(self.file_list):
            ...
            simpledialog.SimpleDialog(
                self.container,
                text="index out-of-bounds, oopsies tehe~",
                buttons=["Ok"],
                title="Usage Dialog").go()
...
class MetadataGuesser:
    ...
    def __init__(self, songloader):
        ...
        self.handler.register(SoundcloudScraper(), r"soundcloud\.com")
        self.songloader = songloader

    def url_load_metadata(self):
        text = simpledialog.askstring("Load Metadata",
...
def main():
    ...
    filemenu.add_command(label="Open File",
                         command=songloader.dialog_load_song)
    filemenu.add_command(label="Open Directory",
                         command=songloader.set_directory)
    ...
    editmenu.add_command(label="Guess Metadata",
                         command=lambda: print("Not Implemented!"))
    editmenu.add_command(label="Load Metadata from URL",
                         command=metadataguesser.url_load_metadata)
    editmenu.add_command(label="Refresh Directory",
                         command=songloader.refresh_filelist)
 ...
 * Only Spotify and Soundcloud urls supported at the moment
Ctrl+l: Copy title and artist to album title and album artists
...
"""
    ...
    helpmenu.add_command(
        ...
        command=lambda: simpledialog.SimpleDialog(
            root, text=USAGE_TEXT, buttons=["Ok"], title="Usage Dialog").go())
    helpmenu.add_command(
        label="About",
        command=lambda: simpledialog.SimpleDialog(
            root, text=HELP_TEXT, buttons=["Ok"], title="Help Dialog").go())
    menubar.add_cascade(label="Help", menu=helpmenu)

    ttk.Button(bottomframe, text="<",
               command=songloader.decrement_index).grid(row=1,
                                                        column=0,
                                                        sticky=W)
    ...
    ttk.Button(bottomframe, text="Save", command=full_save).grid(row=1,
                                                                 column=1,
                                                                 sticky=(W, E))

    ttk.Button(bottomframe, text=">",
               command=songloader.increment_index).grid(row=1,
                                                        column=2,
                                                        sticky=E)
    ttk.Label(bottomframe, textvariable=indexvar).grid(row=0,
                                                       column=1,
                                                       sticky=N)
